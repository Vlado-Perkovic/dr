...
\usetikzlibrary{shapes, calc, positioning, arrows.meta, bending}
\usepackage{pgfplots}
...
\subsection*{Razlika između električnih i mehaničkih stupnjeva}

Prije detaljnijeg opisa koraka, važno je pojasniti razliku između mehaničkih i
električnih stupnjeva. Mehanički stupnjevi ($\theta_m$) predstavljaju stvarni,
fizički zakret osovine motora, pri čemu puni krug uvijek iznosi
\SI{360}{\degree}. Električni stupnjevi ($\theta_e$) odnose se na ciklus
elektromagnetskog polja. Jedan puni ciklus komutacije, koji obuhvaća promjenu
polariteta na svim fazama, uvijek iznosi \num{360} električnih stupnjeva.
...
\begin{itemize}
	\item \textbf{Korak 1:} Struja teče iz faze A (crveni zub statora na slici \ref{fig:aub}) u fazu B (zeleni zub statora na slici \ref{fig:aub}). Faza C je neopterećena. Magnetsko polje statora zauzima određeni položaj prikazan crnom strelicom na slici \ref{fig:aub}. Magnetsko polje statora privlači južni pol stalnog magneta na rotoru te time pomiče rotor za \SI{30}{\degree}.

	      \begin{figure}[h!]
		      \centering

		      \begin{tikzpicture}[
				      stator yoke/.style={draw=gray!80, line width=2pt},
				      stator tooth/.style={draw=gray!80, line width=1.5pt},
				      rotor yoke/.style={draw=gray!80, fill=gray!30, line width=8pt},
				      magnet N/.style={draw=red!30, line width=10pt},
				      magnet S/.style={draw=blue!40, line width=10pt},
				      windingA/.style={fill=red!80},
				      windingB/.style={fill=green!60!black},
				      windingC/.style={fill=blue!80},
				      flux vector/.style={draw=black, line width=2pt, -stealth},
				      % --- New style for the vague lines ---
				      vague line/.style={draw=gray, thin}
			      ]

			      \def\statorOuterRadius{4}
			      \def\statorInnerRadius{3}
			      \def\toothTipRadius{3}
			      \def\statorYokeRadius{1.2}
			      \def\rotorYokeRadius{1.5}
			      \def\magnetRadius{3.75}

			      \def\toothWidthAngle{30}  % Angular width of the tooth body
			      \def\toothTipAngle{45}    % Angular width of the tooth tip/shoe
			      \def\magnetAngle{60}      % Angular width of the magnets
			      \def\arcThickness{0.1}    % Thickness of the arc on the tooth face (1mm = 0.1cm)
			      \def\rotorAngle{105}

			      \def\statorFluxAngle{240}   % Adjust this angle to rotate the stator flux vector
			      \def\fluxVectorLength{5}    % Adjust this length for the vector arrow

			      % --- New variable for the starting angle of the vague lines ---
			      \def\firstLineAngle{0}      % Adjust this for the first line's angle

			      \draw[stator yoke] (0,0) circle (\statorOuterRadius);

			      \foreach \angle/\phaseStyle in {90/windingA, 210/windingB, 330/windingC}
				      {
					      \begin{scope}[rotate=\angle]
						      \def\toothPath{
							      (-\toothWidthAngle/2:\statorYokeRadius)
							      -- (-\toothWidthAngle/2:\toothTipRadius)
							      -- (-\toothTipAngle/2:\toothTipRadius)
							      arc (-\toothTipAngle/2:\toothTipAngle/2:\toothTipRadius)
							      -- (\toothWidthAngle/2:\toothTipRadius)
							      -- (\toothWidthAngle/2:\statorYokeRadius)
							      -- cycle
						      }

						      % use style=<macro> so TikZ applies it as a style, not as a color
						      \path[stator tooth, style=\phaseStyle] \toothPath;

					      \end{scope}
				      }

			      \begin{scope}[rotate=\rotorAngle]
				      \draw[rotor yoke] (0,0) circle (\rotorYokeRadius);

				      \foreach \angle [count=\i] in {45, 135, 225, 315}
					      {
						      \pgfmathsetmacro{\startAngle}{\angle - \magnetAngle/2}
						      \pgfmathsetmacro{\endAngle}{\angle + \magnetAngle/2}

						      \ifodd\i
							      \draw[magnet N] (\startAngle:\magnetRadius) arc (\startAngle:\endAngle:\magnetRadius);
						      \else
							      \draw[magnet S] (\startAngle:\magnetRadius) arc (\startAngle:\endAngle:\magnetRadius);
						      \fi
					      }
			      \end{scope}

			      \draw[flux vector] (0,0) -- (\statorFluxAngle:\fluxVectorLength);

			      % --- Added loop to draw 6 vague lines on top of the picture ---
			      \foreach \i in {0, 1, ..., 5} {
					      \pgfmathsetmacro{\lineAngle}{\firstLineAngle + \i * 60}
					      \draw[vague line] (0,0) -- (\lineAngle:\statorOuterRadius);
				      }
		      ...
		      \begin{subfigure}[b]{0.4\textwidth}%
			      \resizebox{\linewidth}{!}{%
				      \begin{tikzpicture}[
					      ...
					      windingC/.style={fill=blue!80},
					      flux vector/.style={draw=black, line width=2pt, -stealth},
					      angle arrow/.style={draw=black, line width=1pt, -{Stealth[length=2mm, width=2mm]}, shorten >=1pt},
					      angle text/.style={font=\small, fill=white, inner sep=1pt},
					      vague line/.style={draw=gray, thin}
					      ]
					      \def\rotorAngle{105}
					      ...
					      \def\statorFluxAngle{180}   % Adjust this angle to rotate the stator flux vector
					      \def\fluxVectorLength{5}    % Adjust this length for the vector arrow
					      \def\previousFluxAngle{240}      % Previous (or reference) stator flux vector angle (120 - 60 = 60)
					      \def\angleArcRadius{0.8}        % Radius for the angle arc
					      \def\angleTextOffset{0.2}       % Offset for the angle label from the arc
					      \def\firstLineAngle{0}      % Adjust this for the first line's angle
					      ...
					      \foreach \angle/\phaseStyle in {90/windingA, 210/windingB, 330/windingC}
						      {
							      \begin{scope}[rotate=\angle]
								      ...
								      % \fill[style=\phaseStyle, opacity=0.4] % lighter shade of the winding
								      %     (-\toothTipAngle/2:{\statorInnerRadius - \arcThickness})
								      %     arc (-\toothTipAngle/2:\toothTipAngle/2:{\statorInnerRadius - \arcThickness})
								      %     -- (\toothTipAngle/2:\statorInnerRadius)
								      %     arc (\toothTipAngle/2:-\toothTipAngle/2:\statorInnerRadius)
								      %     -- cycle;
							      \end{scope}
						      }
					      ...
					      \draw[flux vector] (0,0) -- (\statorFluxAngle:\fluxVectorLength);
					      \draw[flux vector, dashed, gray] (0,0) -- node[below left] {} (\previousFluxAngle:\fluxVectorLength) coordinate (previousFlux);

					      \draw[angle arrow] (\previousFluxAngle:\angleArcRadius) arc (\previousFluxAngle:\statorFluxAngle:\angleArcRadius);
					      \pgfmathsetmacro{\midAngle}{(\statorFluxAngle + \previousFluxAngle) / 2}
					      \node[angle text] at (\midAngle:\angleArcRadius + \angleTextOffset) {$60^\circ$};
					      \foreach \i in {0, 1, ..., 5} {
							      \pgfmathsetmacro{\lineAngle}{\firstLineAngle + \i * 60}
							      \draw[vague line] (0,0) -- (\lineAngle:\statorOuterRadius);
						      }
			      ...
			      \subcaption{Trenutak promjene koraka}
			      \label{fig:cub_1}
		      \end{subfigure}
		      \hfill
		      \begin{subfigure}[b]{0.4\textwidth}%
			      ...
			      \subcaption{Trenutak kada rotor dostiže magnetsko polje statora}
			      \label{fig:cub_2}
		      \end{subfigure}
		      \caption{Dijagram motora konfiguracije \texttt{3s4p} kada struja teče iz faze C u fazu B}
		      \label{fig:cub}
	      \end{figure}
\end{itemize}
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}[
			auto,
			node distance=1.5cm and 1.1cm, % Reduced distance between nodes (vertical and horizontal)
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.5\textwidth]{Figures/esc.jpeg}
...
\begin{figure}[h!]
	\centering
	\begin{tikzpicture}[
			node distance=1.2cm and 2cm, % Prilagođen razmak za sličan izgled
			process/.style={
					rectangle,
					draw,
					minimum height=1cm,
					text width=4cm, % Širina teksta prilagođena Vašim nazivima
					align=center
				},
			decision/.style={
					diamond,
					draw,
					aspect=2, % Omjer stranica dijamanta
					text width=3.5cm, % Širina teksta dijamanta
					align=center
				},
			terminator/.style={
					rectangle,
		...
		% Definicija čvorova (nodes)
		\node (start)      [terminator]                             {Pokreni motor};
		\node (align)      [process, below=of start, yshift=-0.5cm] {Poravnanje rotora};
		\node (align_time) [decision, below=of align, yshift=-0.5cm]{Prošlo vrijeme utitravanja?};
		\node (open_loop)  [process, below=of align_time, yshift=-0.5cm] {Otvorena petlja + prikupljanje PEMS-a}; % Prilagođen naziv
		\node (comm_done)  [decision, below=of open_loop, yshift=-0.5cm] {Detektirana 3 prolaska kroz nulu?}; % Prilagođen naziv
		\node (running)    [process, below=of comm_done, yshift=-0.5cm] {Zatvorena petlja}; % Prilagođen naziv

		% Crtanje strelica (edges)
		\draw [arrow] (start) -- (align);
		\draw [arrow] (align) -- (align_time);
		\draw [arrow] (align_time) -- node[left] {DA} (open_loop);
		\draw [arrow] (open_loop) -- (comm_done);
		\draw [arrow] (comm_done) -- node[left] {DA} (running);

		% Povratne petlje
		\draw [arrow] (align_time.east) -- ++(2,0) node[above, pos=0.25] {NE} |- (align);
		\draw [arrow] (comm_done.east) -- ++(2,0) node[above, pos=0.25] {NE} |- (open_loop);
		\path [arrow] (running) edge [loop right] node {} (); % Loopback za "Running"

	\end{tikzpicture}
	\caption{Dijagram faza upravljanja komutacijom (prilagođeno prema \cite{NXP_AN2355})}
	\label{fig:faze}
\end{figure}
...
jedinice mikrokontrolera: napredni tajmer \texttt{TIM1} za generiranje signala
PWM-a, općenamjenski tajmeri \texttt{TIM3}, \texttt{TIM15}, \texttt{TIM17} za
...
proglasi trenutak na početku prozora detekcije kao prolazak kroz nulu (slika
\ref{fig:closed_loop_undershoot}). Ovo efektivno skraćuje trajanje trenutačnog
koraka, čime se stator ubrzava i pokušava "pobjeći" rotoru. S druge strane, ako
rotor kasni i prolazak kroz nulu se ne dogodi unutar prozora, sustav označava
kraj prozora kao trenutak detekcije (slika \ref{fig:closed_loop_overshoot}),
čime se sljedeći korak produžuje, a polje statora usporava i "čeka" rotor. Ove
korekcije osiguravaju stabilnost upravljačke petlje.
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{csv/closed_loop_undershoot.pdf}
	\caption{Vremenski dijagram rubnog slučaja ranjenja (prilagođeno prema \cite{NXP_AN2355})}
	\label{fig:closed_loop_undershoot}
\end{figure}
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{csv/closed_loop_overshoot.pdf}
	\caption{Vremenski dijagram rubnog slučaja kašnjenja (prilagođeno prema \cite{NXP_AN2355})}
	\label{fig:closed_loop_overshoot}
\end{figure}
...
na slikama \ref{fig:moj_50} i \ref{fig:simon_50}, jest u gustoći preklapanja
signala PWM-a. Valni oblik komercijalnog rješenja je grublji, što je posljedica
niže frekvencije od \SI{8}{\kilo\hertz}. Suprotno tome, rješenje razvijeno u
ovom radu, s frekvencijom od \SI{30}{\kilo\hertz}, generira znatno finiji valni
oblik s manje izraženim šumom.
\begin{figure}[h!]
	\centering
	\begin{subfigure}[b]{0.9\textwidth}
		\includegraphics[width=\textwidth]{csv/simon_50.pdf}
		\caption{Valni oblik napona\footnote{Napon je negativan jer sklop funkcionira na taj način, ovdje je graf zrcaljen radi lakšeg uočavanja sličnosti} sklopa SimonK}
		\label{fig:simon_50}
	\end{subfigure}
	\begin{subfigure}[b]{0.9\textwidth}
		\includegraphics[width=\textwidth]{csv/moj_50.pdf}
		\caption{Valni oblik napona implementiranog rješenja}
		\label{fig:moj_50}
	\end{subfigure}
\end{figure}
\newpage
