...
detaljno opisuje upravljanje motorom BLDC bez osjetila položaja, s fokusom na komutaciju
u šest koraka. Četvrto poglavlje prikazuje eksperimentalni postav, uključujući
...
\begin{itemize}
	...
	\item \textbf{Oštro preklapanje (eng. \textit{hard chopping}):}
	      ...
	      recirkulira kroz poredne diode (eng. \textit{freewheeling diodes}) suprotnih
	...
	\item \textbf{Komplementarni signal PWM-a:}
	      ...
	      recirkulira kroz tranzistor $S_2$, a kada je signal PWM-a nizak, uključuje se
	      donja sklopka u istom polumostu što se može vidjeti na slici
	      \ref{fig:soft_chopping}. Kada se zeleni signal na tranzistoru $S_1$ spusti u
	      nisko stanje, crveni signal na tranzistoru $S_2$ s malom stankom podigne se u
	      visoko stanje. Ova mala stanka naziva se mrtvo vrijeme (eng. \textit{dead
		      time}) i sprječava kratki spoj na polumostu (eng. \textit{shoot-through}).
	      Uključivanjem donjeg tranzistora na ovaj način postiže se da struja recirkulira
	      kroz tranzistor \cite{TI2015}. \todo[inline]{fale osi na slici}
	      \begin{figure}[h!]
		      \centering
...
karakteristike. \missingfigure{slika motora}

\section{Enkoder}
\label{sec:enkoder}

Za precizno mjerenje stvarne brzine vrtnje i položaja rotora motora korišten je
apsolutni magnetski enkoder \texttt{CUI Devices AMT223C-V}. Iako se primarni
...
struje pri različitim tehnikama primjene signala PWM-a na upravljanje (odjeljak
\ref{sss:pwm}).
...
Za potrebe usporedbe korišteni su komercijalno dostupan sklop \textit{SimonK
	30A}, koji radi na frekvenciji signala PWM-a od \SI{8}{\kilo\hertz}, te sklop
korišten u ovom radu, koji radi na frekvenciji signala PWM-a od
\SI{30}{\kilo\hertz}.
...
\missingfigure{Snimka zaslona osciloskopa: valni oblik faznog napona za sklop \textit{SimonK 30A}.}
...
naglog skoka brzine na samom početku radnog područja.
...
\SI{77}{\%}. Uzme li se u obzir postignuta maksimalna brzina gdje sklop
\textit{SimonK} postiže skoro dvostruko veću brzinu od implementiranog
rješenja, može se donijeti zaključak da je sklop \textit{SimonK} znatno
efikasniji.
...
\textit{SimonK} nudi znatno bolje radne karakteristike u pogledu maksimalne
brzine vrtnje i efikasnosti. Pod opterećenjem, \textit{SimonK} postiže gotovo
dvostruko veću brzinu pri istom strujnom ograničenju, što ukazuje na znatno
efikasniju pretvorbu električne u mehaničku snagu. Glavni uzrok ove razlike
leži u radnoj frekvenciji signala PWM-a. Niža frekvencija od
\SI{8}{\kilo\hertz} kod komercijalnog sklopa rezultira manjim sklopnim gubicima
na tranzistorima, što je ključno za efikasnost pri visokim opterećenjima. S
druge strane, viša frekvencija od \SI{30}{\kilo\hertz}, korištena u ovom radu,
iako osigurava finiji valni oblik napona i bolju linearnost, uzrokuje veće
sklopne gubitke koji umanjuju ukupnu iskoristivost sustava.
...
Provedena usporedba s komercijalno dostupnim rješenjem \textit{SimonK 30A} dala
je ključan uvid u radne karakteristike ostvarene implementacije. Komercijalno
