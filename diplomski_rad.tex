...
rotora i statora. Za to upravljački sklop mora poznavati položaj rotora, što se obično
...
magnetskog polja rotora i rotirajućeg
magnetskog polja statora. Za razliku od klasičnih istosmjernih motora,
rotirajuće polje statora ovdje se stvara elektronički. Taj zadatak obavlja upravljački sklop ESC, koji sekvencijalnim
...
motorom jer se valni oblici pogonskog signala i PEMS-a moraju
poklapati radi minimizacije valovitosti momenta (odjeljak
...
spojene su u zajedničkoj središnjoj točki, zvjezdištu (točka $n$ na
slici \ref{fig:wye}). Fazni napon, $V_{faza}$ [V], niži je od mrežnog
napona, $V_{\text{mreža}}$ [V], prema izrazu $V_{faza} = V_{\text{mreža}} / \sqrt{3}$, a fazna struja, $I_{faza}$ [A], jednaka je
mrežnoj struji.
...
$V_{faza}$ [V], jednak je mrežnom naponu, $V_{\text{mreža}}$ [V], a
fazna struja, $I_{faza}$ [A], niža je od mrežne struje,
$I_{\text{mreža}}$ [A], prema izrazu $I_{faza} = I_{\text{mreža}} /
	\sqrt{3}$.
...
Moment, $\tau$ [Nm], može se izraziti preko izraza $\tau = K_t \cdot
	I_{faza}$, gdje je $K_t$ konstanta momenta [Nm/A], a $I_{faza}$
...
S druge strane, rotacijom se inducira PEMS ($V_{pems}$) koji se
suprotstavlja naponu napajanja i proporcionalan je kutnoj brzini ($\omega$)
prema izrazu $V_{pems} = K_e \cdot \omega$. Porast brzine uzrokuje
porast $V_{pems}$, što smanjuje efektivni napon na namotajima, a time i
struju i raspoloživi moment. Motor dostiže svoju maksimalnu brzinu, odnosno
brzinu praznog hoda, kada se $V_{pems}$ približi naponu napajanja, čime
...
Alternativna strategija je uzorkovanje na kraju niskog
...
PEMS-a jer on nije prisutan pri mirovanju i vrlo malim brzinama. Zbog toga je potrebno motor dovesti do stanja u kojem može očitavati PEMS.
...
u otvorenoj petlji (eng. \textit{open-loop}). Upravljački sklop ESC forsira komutaciju
...
statora: promjer od \SI{22}{\milli\metre} i visinu od \SI{12}{\milli\metre}. Motor ima 14 magnetskih polova na
...
struje pri različitim tehnikama primjene PWM-a na upravljanje (odjeljak \ref{sss:pwm}).
...
jeziku Python za slanje tražene brzine na sklop ESC te primanje trenutačne brzine
rotora preko sučelja \texttt{UART}.
...
signala PWM-a postavljena je na \SI{30}{\kilo\hertz}. Ova frekvencija predstavlja kompromis
...
pojedinim izlazima PWM-a ostvareno je direktnim upisom u registre tajmera čime
se postiže brza kontrola nad aktivnim fazama motora.
...
aktivnog sklapanja, struja recirkulira kroz donji tranzistor polumosta i inducirani napon je
...
\texttt{CMP3\_Pin}), ovisno o trenutačnom komutacijskom koraku.
...
za rad pri brzini od \SI{115200}{bit\per\second}. Prijenos podataka je automatiziran
...
brzinu vrtnje. Istovremeno, sustav periodično šalje trenutačnu procijenjenu
...
\ref{sec:pokretanje}, započinje s poravnanjem rotora. Rotor se može nalaziti u položaju
koji je za \ang{180} stupnjeva pomaknut od željenog smjera magnetskog
polja, što bi rezultiralo neuspješnim poravnanjem. Zbog toga se izvode dva
početna, vremenski dugačka koraka komutacije, svaki u trajanju od približno \SI{260}{\milli\second}.
...
omogućuje detekciju prolaska PEMS-a kroz nulu. Vrijedi napomenuti da, kako rotor
s vremenom sve više kasni, prilika za detekciju prolaska kroz nulu traje svega
nekoliko koraka. Stoga je bitno pouzdano detektirati i zabilježiti sve
...
efektivno skraćuje trajanje trenutačnog koraka, čime se stator ubrzava i pokušava "pobjeći"
rotoru. S druge strane, ako rotor kasni i prolazak kroz nulu se ne dogodi
unutar prozora, sustav označava kraj prozora kao trenutak detekcije, čime se
sljedeći korak produžuje, a polje statora usporava i "čeka" rotor. Ove
...
\begin{sazetak}
	...
	\textit{A2212/13T 1000KV} i razvojnu pločicu \texttt{Nucleo-G071RB} s mikrokontrolerom
	\texttt{STM32G071RB}.
\end{sazetak}
...
\begin{abstract}
	...
	hardware, which includes an electronic speed controller, an \textit{A2212/13T
		1000KV} motor, and a \texttt{Nucleo-G071RB} development board with an \texttt{STM32G071RB}
	microcontroller.
\end{abstract}
