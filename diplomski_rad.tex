...
ustaljenom stanju, kako je teorijski opisano u odjeljku o fazama pokretanja i
rada motora:
\begin{itemize}
	...
	\item \textbf{Rad u otvorenoj petlji (eng. \textit{Open-loop}):} Faza ubrzavanja motora u kojoj se komutacija izvodi prisilno, prema unaprijed definiranoj vremenskoj sekvenci.
...
\ref{pog:bldc_upravljanje_bez_senzora}, koristi se napredni tajmer
\texttt{TIM1}, konfiguriran za generiranje tri para komplementarnih signala
PWM-a. Frekvencija signala PWM-a postavljena je na 30 kHz, što se postiže
vrijednošću registra za automatsko ponovno punjenje (ARR) od 2123 pri taktu
sistemskog sata od 64 MHz bez preskalera. Ova frekvencija predstavlja kompromis
između sklopnih gubitaka na tranzistorima MOSFET i izbjegavanja čujnog područja
rada. Za sprječavanje pojave kratkog spoja (eng. \textit{shoot-through}) unutar
polumostova pretvarača, hardverski je implementirano mrtvo vrijeme (eng.
\textit{dead time}) u trajanju od 50 taktova sistemskog sata. Upravljanje
pojedinim izlazima PWM-a ostvareno je direktnim upisom u registre tajmera
unutar funkcije \texttt{motor\_step}, čime se postiže brza i precizna kontrola
nad aktivnim fazama motora.
...
podataka. Nakon primitka, u prekidnoj rutini \texttt{HAL\_UART\_RxCpltCallback}
ta dva bajta se spajaju u 16-bitnu vrijednost koja predstavlja novu zadanu
brzinu vrtnje. Istovremeno, sustav periodično šalje trenutnu procijenjenu
brzinu vrtnje nazad prema nadređenom sustavu, također koristeći DMA, čime se
omogućuje nadzor rada motora u stvarnom vremenu.
...
pokretanja, prijelaz iz otvorenog u zatvoreni krug upravljanja te na kraju i
regulaciju brzine, prateći teorijski model opisan u poglavlju
...
\ref{pog:bldc_upravljanje_bez_senzora}, izvodi se u dvije početne faze. Prva
faza je \textbf{poravnanje rotora}. Rotor se može nalaziti u položaju koji je
\ang{180} stupnjeva pomaknut od željenog smjera magnetskog polja, što bi
rezultiralo neuspješnim poravnanjem. Zbog toga se izvode dva početna, vremenski
relativno dugačka koraka komutacije. Prvi korak pozicionira rotor, a drugi
potvrđuje njegovo usmjerenje i priprema ga za ubrzanje.

Nakon poravnanja, sustav prelazi u fazu \textbf{rada u otvorenoj petlji}. U
ovoj fazi, komutacija se izvodi prisilno pozivima funkcije
\texttt{motor\_step}, a vremenski razmak između koraka se postepeno smanjuje
prema unaprijed određenoj akceleracijskoj rampi definiranoj u tablici
\texttt{step\_times\_us}. Kako se frekvencija komutacije povećava, rotor zbog
svoje tromosti počinje kutno zaostajati za rotirajućim magnetskim poljem
statora. Upravo to relativno gibanje uzrokuje promjenu magnetskog toka potrebnu
za indukciju PEMS-a koji se, nakon nekoliko koraka, može validno očitati.
...
Sinkronizacija komutacije s položajem rotora u zatvorenoj petlji, temeljena na
metodi komutacije u šest koraka (odjeljak \ref{sec:six-step}), ostvarena je
preciznim vremenskim slijedom događaja. Ciklus započinje detekcijom prolaska
...
period između dva prolaska kroz nulu te, koristeći taj podatak i empirijski
određen koeficijent faznog pomaka, određuje vremensku odgodu do iduće
komutacije. Izračunata vrijednost odgode upisuje se kao novi period u
\texttt{ARR} registar tajmera \texttt{TIM3}. Po isteku zadanog vremena,
\texttt{TIM3} generira prekid u kojem se postavlja zastavica
...
nulu se dogodi \textit{prije} prozora za detekciju (stanje
\textit{undershoot}), sustav to registrira i skraćuje trajanje trenutnog koraka
kako bi polje statora "sustiglo" rotor. Ako rotor kasni i prolazak kroz nulu se
ne dogodi unutar prozora (stanje \textit{overshoot}), sustav označava kraj
prozora kao trenutak detekcije, čime se sljedeći korak produžuje i polje
statora "čeka" rotor. Ove korekcije osiguravaju stabilnost upravljačke petlje.
...
modelu prikazanom u poglavlju \ref{pog:bldc_upravljanje_bez_senzora}.

\subsubsection{Procjena brzine vrtnje}
\label{sssec:procjena_brzine}

Brzina rotora izračunava se na temelju perioda komutacije. Vrijeme između dvije
uzastopne detekcije prolaska PEMS-a kroz nulu (\texttt{zc\_period}) odgovara
periodu od 60 električnih stupnjeva. Kako bi se ublažile oscilacije,
primijenjen je digitalni filtar tipa pomičnog prosjeka (eng. \textit{moving
	average}). U red (\texttt{queue\_t}) veličine 6 pohranjuje se zadnjih šest
...
polova motora (7 za A2212), a $T_{avg}$ prosječan broj taktova tajmera za jedan
komutacijski korak. U kodu je ova jednadžba implementirana korištenjem
konstante \texttt{RPM\_CONSTANT}.
