...
\begin{tikzpicture}[
		auto,
		node distance=2.5cm and 2cm, % Default distance between nodes (vertical and horizontal)
		...
		block/.style={
				...
				minimum height=1cm,
				minimum width=2cm,
				text width=1.8cm, % Ensures text wraps if needed
				align=center
			},
		sum/.style={
				draw,
	...
	\node[block, right=of sum, name=controller] {PI Regulator};
	\node[block, right=of controller, name=plant] {BLDC Motor};
	\node[io, right=of plant, name=output] {};
	\node[io, below=of plant, name=feedback_point] {};
	\draw[->] (input) -- node[above] {$\omega_{ref}(s)$} (sum);
	\draw[->] (sum) -- node[above] {$e(s)$} (controller);
	\draw[->] (controller) -- node[above] {$u(s)$} (plant);
	\draw[->] (plant) -- node[above] {$\omega(s)$} (output);
	\draw[->] (output) -- ++(0.5,0) |- (feedback_point) -| (sum);
	\node at (sum.north) [above=0.1cm] {$+$};
	\node at (sum.west) [left=0.1cm] {}; % Empty node for alignment if needed
